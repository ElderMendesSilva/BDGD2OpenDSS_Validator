...
"""
import json
# O `sys.path` VEM ANTES DO IMPORT, e nao depois. Enquanto estes
# executaveis moravam na raiz, o proprio diretorio de trabalho achava
# `bdgd2dss` e a ordem nao importava; de `etapas/` ela importa, e o
# defeito so aparece ao rodar de outra pasta.
import os
import sys
...
def _grava(secao, valores):
    ...
    try:
        # `newline=` EXPLICITO: no Linux sairia LF e no Windows CRLF, e a
        # regra do projeto e que arquivo de texto nao dependa do sistema. Este
        # passou despercebido enquanto o modulo estava na raiz, fora da
        # varredura do teste.
        with open(_MEMORIA, 'w', encoding='utf-8',
