...
def _bom(**troca):
    ...
    v = {'veredicto': 'OK', 'cargas_sem_tensao': 0, 'n_cargas': 1000}
    e = {'perdas_pct': 3.5}
...
class TestClassificacao(unittest.TestCase):
    ...
    def test_nao_converge_reprova_tudo(self):
        v, e, g, fic, fdia, extra = _bom(v_={'veredicto': 'NAO_CONVERGE'})
    ...
    def test_ausencia_de_medida_nao_e_aprovacao(self):
        """O defeito real: campo vazio não estava na lista de falhas, então
        «Fecha eletricamente» dizia «sim» numa base em que a etapa nem rodou."""
        v, e, g, fic, fdia, extra = _bom(v_={'veredicto': None})
...
class TestParaQueServe(unittest.TestCase):
    ...
    def test_o_diagnostico_e_sempre_um_uso_valido(self):
        for troca in ({}, {'v_': {'veredicto': 'NAO_COMPILA'}},
                      {'v_': {'veredicto': None}}):
